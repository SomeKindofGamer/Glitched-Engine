...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (FlxG.save.data.fpsCap > 290)
			(cast (Lib.current.getChildAt(0), Main)).setFPSCap(8000);
		...
		healthBar.createFilledBar(0xFFFF0000, 0xFFADD8E6);
		// healthBar
		add(healthBar);

		// Add Song Name
		songText = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy"), 16);
		songText.y = 690;
        ...
        */
		...
		replayTxt.setFormat(Paths.font("vcr.ttf"), 55, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		botPlayState = new FlxText("AUTOPLAY", 20);
		botPlayState.y - 175;
		botPlayState.x -20;
		...
		if(FlxG.save.data.botplay && !loadRep) add(botPlayState); trace('Autoplay Loaded');
		...
		if (generatedMusic)
			{
				...
				{	
					if (!daNote.mustPress && !daNote.isSustainNote && FlxG.save.data.MiddleScroll)
						daNote.setGraphicSize(Std.int(daNote.width * 0.65));
					else if (!daNote.mustPress && daNote.isSustainNote && FlxG.save.data.MiddleScroll)
